...
class ForwardMessageHandler:
    def __init__(self, db: Database, application, bot: 'TelegramBot'):  # 使用字符串类型注解
        self.db = db
    ...
    async def _get_reply_method(self, update: Update, context: ContextTypes.DEFAULT_TYPE, is_button: bool, default_chat_id: int):
        ...
        return lambda *args, **kwargs: context.bot.send_message(chat_id=default_chat_id, *args, **kwargs)

    async def get_current_chat_with_validation(self, admin_id: int) -> Tuple[Optional[int], Optional[UserInfo], Optional[str]]:
        try:
            async with asyncio.timeout(5):
                ...
                async with self.db.transaction():
                    ...
                    if not await self.db.is_verified(target_user_id):
                        logger.warning(f"User {target_user_id} is not verified for admin {admin_id}, clearing chat state")
                        await self.clear_chat_state(admin_id)
    ...
    async def forward_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user.id == Config.ADMIN_ID and bot and hasattr(bot, 'waiting_user_id') and user.id in bot.waiting_user_id:
            logger.debug(f"Admin {user.id} is waiting for user ID input, command: {bot.waiting_user_id[user.id]}")
            if not message.text:
                ...
                return
            if await bot._handle_interactive_user_id(update, context):
        ...
        if user.id == Config.ADMIN_ID:
            ...
            try:
                ...
                await self.bot.send_temp_message(
                    message.chat_id,
        ...
        async with self.db.transaction():
            ...
            if not is_verified:
                ...
                await self.bot.reply_error(update, "请先完成人机验证，使用 /start 开始")
                return
        ...
        try:
            ...
            await self.bot.send_temp_message(message.chat_id, "消息已转发")
    ...
    async def switch_chat(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, is_button: bool = False):
        admin_id = update.effective_user.id
        reply_method = await self._get_reply_method(update, context, is_button, admin_id)

        if admin_id != Config.ADMIN_ID:
            await reply_method("仅限管理员使用", parse_mode=None)
        ...
        try:
            escaped_nickname = self.escape_markdown_v2(user_info.nickname or "未知用户") if self.escape_markdown_v2 else user_info.nickname
            await reply_method(
                f"已将对话目标切换为“[{escaped_nickname}](tg://user?id={user_id})”",
                parse_mode="MarkdownV2"
            )
            logger.info(f"Admin {admin_id} switched to user {user_id}")
        except Exception as e:
            logger.error(f"Failed to send switch confirmation for admin {admin_id} to user {user_id}: {str(e)}", exc_info=True)
