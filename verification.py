...
class MathVerification:
    ...
    def _generate_problem_components() -> Tuple[int, int, int, int, int, int, str, str]:
        ...
        denominator = random.choice([n for n in numbers if abs(n) != abs(numerator) and math.gcd(abs(numerator), abs(n)) == 1])

        base = random.choice(numbers)
        exponent = random.choice(powers)
        sqrt_coeff = random.choice(constants)
        ...
        return numerator, denominator, base, exponent, sqrt_coeff, sqrt_number, operator1, operator2

    @staticmethod
    def _compute_answer(numerator: int, denominator: int, base: int, exponent: int, 
                       sqrt_coeff: int, sqrt_number: int, op1: str, op2: str) -> float:
        """计算正确答案，明确运算优先级。"""
        ops = {"+": lambda x, y: x + y, "-": lambda x, y: x - y, "*": lambda x, y: x * y}
        
        fraction = numerator / denominator
        power_term = base ** exponent
        sqrt_term = sqrt_coeff * math.sqrt(sqrt_number)
    ...
    def _generate_question_string(numerator: int, denominator: int, base: int, exponent: int, 
                                 sqrt_coeff: int, sqrt_number: int, op1: str, op2: str) -> str:
        ...
        if sqrt_coeff == 1:
            ...
            sqrt_str = f"({sqrt_coeff}√{sqrt_number})"
        else:
            sqrt_str = f"{sqrt_coeff}√{sqrt_number}"
    ...
    def generate_question() -> Tuple[str, float, List[float]]:
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                numerator, denominator, base, exponent, sqrt_coeff, sqrt_number, op1, op2 = components
                
                question = MathVerification._generate_question_string(
                    numerator, denominator, base, exponent, sqrt_coeff, sqrt_number, op1, op2
                )
